...
void drawBlur( drawData drawInformation ) {
	...
	while (j < ymax) {
		...
		Uint32 gvalue = 0;
		Uint32 bvalue = 0;
		Uint32 radd = 0; Uint32 rsub = 0;
		Uint32 gadd = 0; Uint32 gsub = 0;
		Uint32 badd = 0; Uint32 bsub = 0;
...
void copyBufferScaled(drawData drawInformation) {
	...
	int sourceAddress;
	Uint32 * pixelsSource = sourceBuffer -> screen;
	Uint32 * pixelsTarget = targetBuffer -> screen;
...
/*void copyBufferSectionWithAlpha(drawData drawInformation) {
	...
	int sourceStart = ySource * ( sourceWidth ) + xSource;
	int targetStart = yTarget * ( targetWidth ) + xTarget;
	...
	if ( (targetBuffer -> windowHeight) - yTarget < height ) {
		height = (targetBuffer -> windowHeight) - xTarget;
	}
	if ( (sourceBuffer -> windowHeight) - ySource < height ) {
		height = (sourceBuffer -> windowHeight) - xSource;
	}

	int i,j;
	for ( j = 0; j < height; j++ ) {
		for ( i = 0; i < width; i++ ) {
			uint32_t sourcePix = pixelsSource[ sourceStart + i ];
			uint32_t targetPix = pixelsTarget[ targetStart + i ];
			...
			pixelsTarget[ targetStart + i ] = red + green + blue;
		}
		targetStart += targetWidth;
...
void copyBufferSectionWithAlpha(drawData drawInformation) {
	...
	Uint32 * sourcePixels = (Uint32 *)(sourceBuffer -> screen);
	Uint32 * targetPixels = (Uint32 *)(targetBuffer -> screen);
